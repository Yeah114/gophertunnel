...
// PongData holds all fields parsed from unconnected pong data.
type PongData struct {
	// Edition is the edition of Minecraft of the server, always "MCPE".
	Edition string
	// ServerName is the name or MOTD of the server, as shown in the server list.
	ServerName string
	// Protocol is the protocol version of the server. It is used to determine the Minecraft version of the server.
	Protocol int32
	// Version is the Minecraft version of the server, such as "1.18.10".
	Version string
	// PlayerCount is the current amount of players displayed in the list.
	PlayerCount int
	// MaxPlayers is the maximum amount of players in the server. If set to 0, MaxPlayers is set to
	// PlayerCount + 1.
	MaxPlayers int
	// ServerID is the unique ID of the server, which is used for LAN servers. It is a random string generated by the server on startup.
	ServerID string
	// ServerSubName is the sub-name or sub-MOTD of the server, as shown in the friend list.
	ServerSubName string
	// GameMode is the game mode of the server, as shown in the friend list.
	GameMode string
	// GameModeID is the ID of the game mode of the server, as shown in the friend list.
	GameModeID int
	// IPv4Port is the IPv4 port of the server.
	IPv4Port int
	// IPv6Port is the IPv6 port of the server.
	IPv6Port int
}
...
// ParsePongFullData parses unconnected pong data into all known fields.
func ParsePongFullData(pong []byte) PongData {
	...
	if len(frag) > 1 {
		data.ServerName = frag[1]
	}
	if len(frag) > 2 {
		proto, _ := strconv.ParseInt(frag[2], 10, 32)
		data.Protocol = int32(proto)
	}
	if len(frag) > 3 {
		data.Version = frag[3]
	}
	if len(frag) > 4 {
		data.PlayerCount, _ = strconv.Atoi(frag[4])
	}
	if len(frag) > 5 {
		data.MaxPlayers, _ = strconv.Atoi(frag[5])
	}
	if len(frag) > 6 {
		data.ServerID = frag[6]
	}
	if len(frag) > 7 {
		data.ServerSubName = frag[7]
	}
	if len(frag) > 8 {
		data.GameMode = frag[8]
	}
	if len(frag) > 9 {
		data.GameModeID, _ = strconv.Atoi(frag[9])
	}
	if len(frag) > 10 {
		data.IPv4Port, _ = strconv.Atoi(frag[10])
	}
	if len(frag) > 11 {
		data.IPv6Port, _ = strconv.Atoi(frag[11])
	}
	return data
}
...
func ParsePongData(pong []byte) ServerStatus {
	data := ParsePongFullData(pong)
	return ServerStatus{
		ServerName:    data.ServerName,
		ServerSubName: data.ServerSubName,
